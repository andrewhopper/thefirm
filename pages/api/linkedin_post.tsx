...
export default async function handler(
    ...
    if (req.method === 'GET') { // Check if the request method is POST
        ...
        if (typeof final_topic === 'string') {
            ...
            let producer_agent = social_media_marketer;
            if (agent == 'ux_researcher') {
                ...
                producer_agent = social_media_marketer;
            }

            // res.status(200).json({ result });

            const artifact_type = 'LinkedInPost';
            const prompt = promptComposer(task, artifact_type, artifact_type, andrew_context, requester_actor, producer_agent, "JSON");


            if (dryrun === "true") {
                res.setHeader('Content-Type', 'text/html');
                res.status(200).json({ prompt: prompt, result_type: artifact_type, result: null });
                return;
            }
            else {
